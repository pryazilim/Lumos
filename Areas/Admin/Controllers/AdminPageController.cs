...
public class AdminPageController : Controller
{
  ...
  [HttpPost]
  public JsonResult SliderManagement(int id, int Type, int OrderBy, string Title_tr, string Title_en, string UnderTitle_tr, IFormFile Logo)
  {
    // validation
    // processing
    // generating result
    ...
    try
    {
      ...
      for (int i = 0; i < LanguageList.Count; i++)
      {
        ...
        var Title = Title_tr;
        var _UnderTitle = "UnderTitle_" + variableCode;
        var _ButtonTitle = "ButtonTitle_" + variableCode;
        var _ButtonUrl = "ButtonUrl" + variableCode;
      ...
      if (Logo != null)
      {
        ...
        var _fileNameUniq = Guid.NewGuid().ToString("N").Take(5);

        var img = Image.FromStream(Logo.OpenReadStream());

        LogoImgPath = $"{_fileName}_{_fileNameUniq}{_fileExtension}";

        string path = Path.Combine((@"/UploadFile/Slider"), LogoImgPath);
